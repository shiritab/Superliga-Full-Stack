  ...
  <div>
    ...
    <b-table fixed striped hover :items="future_games" :fields="future_fields">
        ...
        <template #cell(favorite)="data">
            <b-icon-heart v-if="$root.store.username && !data.item.favorite" @click="setFavorite(data.item.game_id); data.item.favorite=true;" font-scale="1.5"></b-icon-heart>
            <b-icon-heart-fill v-if="$root.store.username && data.item.favorite"  font-scale="1.5" variant="danger"></b-icon-heart-fill>
        </template>
        <template #cell(home_team)="data">
            <router-link class="links" :to="{ name: 'team ticket', params:{teamId: data.item.home_team_id}}">
                {{data.item.home_team}}
...
export default {
    ...
    methods:{
        async setFavorite(game_id){
            console.log(`favoriting ${game_id}`);
            this.$emit('click', game_id);
